...
def imuPublisher():
    ...
    while not rospy.is_shutdown():
        try:
            ...
            # Save the publishing interval time to frame_id            
            msg.header.frame_id = str(time.time() - timer)
            timer = time.time()
            
            # Save accelometer data to ROS message
    ...
    # Inform that the publisher is exiting
    print("Exiting imu publisher...")
    sys.exit()
        
    
# Main program with two threads: imu capture and ros publish
